...
const ProductDetail = () => {
    ...
    const getPrice = () => {
        ...
        if (selectedColor && product.subPrice) {
            const colorPrice = product.subPrice.find(p => p.color === selectedColor);
    ...
    const handleThumbnailClick = (imageData) => {
        ...
        if (imageData.subCategory) {
            ...
            if (imageData.subCategory.size) setSelectedSize(imageData.subCategory.size);
            if (imageData.subCategory.color) setSelectedColor(imageData.subCategory.color);
        } else {
        ...
        <Container style={{marginTop: '160px'}}>
            <ProductSection>
                        ...
                        {allImages.map((imageData, index) => (
                            ...
                            >
                                <img
                                    ...
                                    alt={`${product?.name} ${imageData.isMain ? 'main' : 'variant'} ${index + 1}`}
                                    onError={(e) => {
                                        e.target.onerror = null;
                                        e.target.src = '/placeholder.jpg';
                                    }}
                ...
                <ProductInfo>
                    ...
                    <AddToCart>Thêm vào giỏ hàng</AddToCart>
                </ProductInfo>
            </ProductSection>
                            ...
                            <ProductCard key={relatedProduct.slug}>
                                <Link to={`/products/${relatedProduct.slug}`}>
                                    ...
                                    <ProductCardInfo>
                                        ...
                                        <p>Chỉ từ {relatedProduct.price.toLocaleString()}đ</p>
                                    </ProductCardInfo>
                                </Link>
                            </ProductCard>
                        ))}
